...
from ..models import Channel, User, Message, Dm, db
...
def add_new_obj():
	...
	if new_type == 'channel':
		...
		if name:
            ...
            # return success
			return jsonify({'name': new_channel[1].channel_name, 'success': True,
                            'error': 'null'})
		else:
		...
		if name:
			...
			return jsonify({'name': new_dm[1].get_name(new_dm[2]), 'success': True, 'error': 'null'})

		return jsonify({'name': '', 'success': True,
			'error': 'null'})
	else:
		pass
...
def send_message_to_dm(data):
	...
	if other_user and user:
		dm1 = Dm.query.filter_by(user_one=user.id, user_two=other_user.id).first()
		dm2 = Dm.query.filter_by(user_one=other_user.id, user_two=user.id).first()
		if dm1:
			DM = dm1
		elif dm2:
			DM = dm2
				...
				# return error
				emit('Error', {'error': 'DM does not exists'},
					broadcast=False)
	else:
		emit('Error', {'error': 'Invalid message'}, broadcast=False)
